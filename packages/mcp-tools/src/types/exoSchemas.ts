...
// ComponentProps/ExperienceProps via indexed-access aliases.
export type {
  ComponentTypeViewport,
  ComponentTypeContentProperty,
  ComponentTypeDesignProperty,
...
export type ComponentTreeDesignPropertyValue =
  ComponentNodeV2['designProperties'][string];
...
export type DataTypeDefinition = Extract<
  ComponentTypeContentProperty,
  { type: 'Array' }
>['items'];
...
>;
export type RichTextDataTypeDefinition = Extract<
  DataTypeDefinition,
  { type: 'RichText' }
>;
export type ArrayDataTypeDefinition = Extract<
  DataTypeDefinition,
  { type: 'Array' }
>;
export type RecordDataTypeDefinition = Extract<
  DataTypeDefinition,
  { type: 'Record' }
>;
export type TypeRefDataTypeDefinition = Extract<
  DataTypeDefinition,
  { type: 'TypeRef' }
>;
export type LiteralDataTypeDefinition = Extract<
  DataTypeDefinition,
  { type: 'Literal' }
>;
...
export type StringDesignProperty = Extract<
  ComponentTypeDesignProperty,
  { type: 'String' }
>;
export type BooleanDesignProperty = Extract<
  ComponentTypeDesignProperty,
  { type: 'Boolean' }
>;
...
const URN_EXAMPLES: Partial<Record<string, string>> = {
  'Contentful:Component':
    'crn:contentful:::experience:spaces/{spaceId}/environments/{envId}/components/{id}',
  'Contentful:ExperienceFragment':
    'crn:contentful:::experience:spaces/{spaceId}/environments/{envId}/experienceFragments/{id}',
  'Contentful:ExperienceTemplate':
    'crn:contentful:::experience:spaces/{spaceId}/environments/{envId}/experienceTemplates/{id}',
...
export const ComponentResourceLinkSchema = resourceLinkSchema(
  'Contentful:Component',
);
export const ExperienceFragmentResourceLinkSchema = resourceLinkSchema(
  'Contentful:ExperienceFragment',
);
export const ExperienceTemplateResourceLinkSchema = resourceLinkSchema(
  'Contentful:ExperienceTemplate',
);
...
export const DesignPropertyPointerValueSchema =
  z.custom<`$designProperties/${string}`>(
...
// ── Slot definition ───────────────────────────────────────────────────────────
// Matches CMA.js ComponentSlotDefinition — allowed children are constrained
// via allowedResources, not a bare component ID array.
...
export const SlotDefinitionSchema = z.object({
  ...
  allowedResources: z
    .array(
      z.object({
        type: z.literal('Contentful:Component'),
        source: z.literal(COMPONENT_TYPE_ALLOWED_RESOURCE_SOURCE),
        allowedTypes: z
          .array(z.string())
          .describe('IDs of components allowed in this slot'),
      }),
    )
    .optional()
    .describe('Constrains which components may be placed in this slot'),
}) satisfies z.ZodType<ComponentSlotDefinition>;

// ── Component tree ────────────────────────────────────────────────────────────
// Matches CMA.js TreeNodeV2 = ComponentNodeV2 | ExperienceFragmentNode | SlotNode
...
//
// The exported schema names deliberately drop the SDK's V2 suffix: CMA.js notes that
// TreeNodeV2 will be renamed back to TreeNode once the migration completes, so mirroring
// the suffix here would only guarantee a second rename.

const ExperienceFragmentNodeSchema = z.object({
  id: z.string().describe('Node identifier'),
  name: z.string().optional().describe('Optional display name'),
  nodeType: z
    .literal('ExperienceFragment')
    .describe('Must be "ExperienceFragment"'),
  experienceFragment: ExperienceFragmentResourceLinkSchema.describe(
    'Resource link to the referenced experience fragment',
  ),
}) satisfies z.ZodType<ExperienceFragmentNode>;
...
export const TreeNodeSchema: z.ZodType<TreeNodeV2> = z.lazy(() =>
  z.union([ComponentNodeSchema, ExperienceFragmentNodeSchema, SlotNodeSchema]),
);

const ComponentNodeSchema: z.ZodType<ComponentNodeV2> = z.lazy(() =>
  z.object({
    id: z.string().describe('Node identifier'),
    name: z.string().optional().describe('Optional display name'),
    nodeType: z.literal('Component').describe('Must be "Component"'),
    component: ComponentResourceLinkSchema.describe(
      'Resource link to the component to render',
    ),
    contentProperties: z
      .union([
...
// - ExoMetadataProps {tags?, concepts?} for Component / ExperienceTemplate
// - ExperienceMetadataProps {tags?, concepts?, name?} for Experience / ExperienceFragment
...
export const ExoMetadataSchema = z.object({
  tags: z
    .array(TagLinkSchema)
    .optional()
    .describe('Tags attached to this entity'),
  concepts: z
...
// whose entries are Array<ExperienceFragmentNode | InlineExperienceFragmentNode>.

export const DataAssemblyResourceLinkSchema = resourceLinkSchema(
  'Contentful:DataAssembly',
);
