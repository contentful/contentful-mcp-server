...
export function registerEntriesTools(server: McpServer) {
  ...
  server.tool(
    'update_entry',
    "Update an existing entry. The handler will merge your field updates with the existing entry fields, so you only need to provide the fields you want to change, with the exceptin of multilingual entries. You will need to provide all existing locale data including the updated locale data for multilingual entries. IMPORTANT: All field values MUST include a locale key (e.g., 'en-US') for each value, like: { title: { 'en-US': 'My Updated Title' } }. Every field in Contentful requires a locale even for single-language content. When updating multilingual entries in Contentful, always include all existing locales in the update to prevent overwriting. RICH TEXT FIELDS: When updating rich text fields, ALL text nodes MUST include a 'marks' property (can be empty array [] for no formatting). Text nodes with formatting need appropriate marks: { nodeType: 'text', value: 'Bold text', marks: [{ type: 'bold' }], data: {} }.",
